...
def get_count_pileup(filename):
    ...
    minRatio = .1
    maxRatio = .9
    pileup = pd.DataFrame(columns=['Chr', 'Pos', 'Ref', 'depth', 'read', 'Qual'])
    for chunk in pd.read_csv(filename, sep='\t', header=None,  names=['Chr', 'Pos', 'Ref', 'depth', 'read', 'Qual'], chunksize=chunksize):
        chunk['read'] = chunk['read'].str.upper()
        for base in 'ACTG':
            chunk[base] = chunk['read'].str.count(base)
        chunk['AltSum'] = chunk[['A', 'C', 'T', 'G']].max(axis=1)
        chunk['AltRatio'] = chunk['AltSum'] / chunk['depth']	
        hotspot_df = chunk.query('(AltSum >= @minAlt) and (@minRatio <= AltRatio <= @maxRatio)')
        pileup = pileup.append(hotspot_df)
...
def get_HDR_info(mut_row, hotspot_df, bam_df, MINSIM=0.9, min_q=25):
    ...
    print(get_adjacent_HDR(mut_row, hotspot_df))
...
def getHDR(bam_file, filter_df, pileup_file='', MINSIM=.90, padding=100, min_HDR_count=1, min_q=25):
    ...
    hotspot_df = get_count_pileup(pileup_file)
    print(
    ...
    if (filter_HDR.empty):
        filter_df['HDRcount'] = 0
        filter_df['HDRinfo'] = 'no HDR'
    else:
...
def masterHDR(filter_df, tumor_bam='', pileup_file='', min_q=25, MINSIM=.90, padding=100, min_HDR_count=1):
